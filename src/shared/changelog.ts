export const CHANGELOG: { version: string; notes: string[] }[] = [
  {
    ...
    notes: [
      ...
      'The mouse cursor now shows when it leaves the radial menu disc.',
      "Plugin icons in the radial menu now match the theme's icon color.",
      ...
      'Fixed hotkeys sometimes staying dead after tabbing out of a tool window.',
    ],
  },
  {
    version: '1.0.3',
    notes: [
      ...
      'Added price-check support for Sanctified items.',
      ...
      'Refreshed the Beasts regex data.',
      ...
      'Fixed Counter-Thaumaturgy heist contracts failing their price check outright.',
      ...
      'Fixed moving an item between tiers making the tier it left show unrelated items.',
      'Fixed Base mode price checks ignoring quality on items above 20%.',
      "Fixed Tulfall's added cold damage per Power Charge losing its price check row.",
      'Fixed Use Saved Regex hotkeys pasting mangled input into the game.',
      'Fixed trade searches the API rejects showing "No listings found" instead of the reason.',
      'Fixed logged-out Mercenary Warrant price checks coming back empty.',
    ],
  },
  {
    version: '1.0.2',
    notes: [
      ...
      'Added price-check support for Mercenary Warrants, including their skills and supports.',
      "Trade listings now show a Mercenary Warrant's skill icons, with its supports on hover.",
      'Tried to make Mercenary Warrant price checks smarter.',
      'Added price-check support for Vestigial items.',
      'Uniques that donate a Vestigial mod now show a chip listing what they hand over.',
      'Price check now badges influence, Delve and Temple mods with where they came from.',
      ...
      'Updated bulk item price check to no longer be about the listings since they are easily manipulated, but instead be about a pretty lil chart.',
      'Buyout price can now be set to any currency the trade site offers, not just the chaos/divine pair.',
      'Added a search tab to the overlay navbar.',
      'Added a "Restart now" button to the Developer settings section (s/o cccarv82!).',
      'Items now copy with a plain Ctrl+C instead of Ctrl+Alt+C, so no more stuck Alt-pinned tooltip in game.',
      'Price check no longer stalls for a second or two on mod-heavy items.',
      ...
      'Refreshed the item tier data.',
      'Added item art for the 3.29 items.',
      "Fixed weapon implicits matching the global stat instead of the weapon's own local one, so no listings came back.",
      "Fixed long mods that wrap onto a second line starting with a capital (like Kitava's Thirst) being read as two mods and matching the wrong stat.",
      'Fixed a mod disappearing from price check when its affix block mixes a multi-line mod with an extra line.',
      "Fixed price check adding a bogus extra row when one mod's text sits inside a longer one.",
      "Fixed chance-and-threshold mods like Foulborn Kitava's Thirst averaging their two numbers together, so Exact Values found nothing.",
      'Fixed synthesis implicits that hide their chance but keep a duration (like Intimidate for 4 seconds) failing to match.',
      'Fixed Elder hybrid "Socketed Gems are Supported by" searches inheriting the companion mod\'s tier range instead of the support level.',
      ...
      "Fixed Facetor's Lens reading 750 000 000 stored experience as 750 in clients that group large numbers with spaces.",
      ...
      'Fixed unique price-checks pricing against Foulborn copies.',
      'Fixed unenchanted Heist Blueprints price-checking against Enchanted Armaments.',
      'Fixed unidentified map searches dropping visible implicits, so Elder/Shaper influence, guardian and citadel maps searched as plain T16s.',
      'Fixed plain originator maps price-checking against Elder copies.',
      ...
      'Fixed PoE2 Charm price checks coming back empty.',
      'Fixed the Corrupted chip missing on Abyss Jewels, tinctures, Heist gear and PoE2 flasks.',
      ...
      'Fixed Reliquary Keys price-checking as web listings instead of on the Currency Exchange.',
      ...
      'Fixed a PoE2 private league selection not sticking; it got rewritten back to the challenge league on save (s/o eniner!).',
      'Fixed new profiles starting on the previous league instead of the current one.',
      'Fixed the League setting still showing the old league after a rotation.',
      'Fixed the PoE2 league list never refreshing from the trade site.',
      'Fixed macros not reloading properly when switching between PoE1 and PoE2.',
  ...
  },
  {
    ...
    notes: [
      ...
      "Temporarily hid the socket color calculator while the league's new chromatic orb formula shakes out.",
      ...
      "Fixed hotkeys sometimes firing while the game wasn't focused, or acting on the wrong game when both PoE1 and PoE2 were open (s/o DZ!).",
    ],
  },
  {
    version: '0.9.16',
  ...
  // before upstream shipped its versions. Leave it.
  {
    ...
    notes: [
      'Scalpel now only types chat commands and filter reloads into the game while Path of Exile is focused, so a hotkey can no longer accidentally paste your clipboard into chat or another window.',
